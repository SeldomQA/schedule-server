...
export const apiDataToRenderData = (item: IStubMapping) => {
  ...
  item.metadata.render.request.urlMatchType =
    (item.request.url && 'url') ||
    (item.request.urlPattern && 'urlPattern') ||
    (item.request.urlPath && 'urlPath') ||
    (item.request.urlPathPattern && 'urlPathPattern') ||
    'any'
  item.metadata.render.request.url =
    item.request.url ||
    item.request.urlPattern ||
    item.request.urlPath ||
    item.request.urlPathPattern ||
    ''
  // Query Params
  if ($(item.request.queryParameters)) item.request.queryParameters = {}
  item.metadata.render.request.queryParameters = apiDataToRenderDataWithMatch(
    item.request.queryParameters
  )

  // Request Headers
  item.metadata.render.request.authType =
    (item.request.basicAuthCredentials && 'basicAuth') || 'none'
  if ($(item.request.basicAuthCredentials))
    item.request.basicAuthCredentials = {
      username: '',
      password: ''
    }
  ...
  // Request Body
  item.metadata.render.request.bodyType =
    (item.request.bodyPatterns && 'raw') ||
    (item.request.multipartPatterns && 'form-data') ||
    (item.request.formParameters && 'x-www-form-urlencoded') ||
    'none'
  if ($(item.request.bodyPatterns)) item.request.bodyPatterns = []
  item.metadata.render.request.bodyPatterns = apiDataToRenderDataWithMatch(
    item.request.bodyPatterns
  )
  ...
  item.metadata.render.request.formParameters = apiDataToRenderDataWithMatch(
    item.request.formParameters
  )

  // Request Custom Matcher
  if ($(item.request.customMatcher))
    item.request.customMatcher = {
      name: '',
      parameters: {}
    }
  ...
  // 3. Response
  // 3.1 General Info
  item.metadata.render.response.responseType =
    (item.response.proxyBaseUrl && 'proxy') ||
    (item.response.fault && 'fault') ||
    ((item.response.body ||
      item.response.jsonBody ||
      item.response.base64Body ||
      item.response.bodyFileName) &&
      'direct') ||
    'none'
  item.metadata.render.response.delayType =
    (item.response.fixedDelayMilliseconds && 'fixedDelayMilliseconds') ||
    (item.response.chunkedDribbleDelay && 'chunkedDribbleDelay') ||
    (item.response.delayDistribution &&
      item.response.delayDistribution.type === 'lognormal' &&
      'lognormal') ||
    (item.response.delayDistribution &&
      item.response.delayDistribution.type === 'uniform' &&
      'uniform') ||
    'none'

  if ($(item.response.chunkedDribbleDelay))
    item.response.chunkedDribbleDelay = {
      numberOfChunks: 1,
      totalDuration: 0
    }
  if ($(item.response.fixedDelayMilliseconds)) item.response.fixedDelayMilliseconds = 0
  if ($(item.response.delayDistribution))
    item.response.delayDistribution = {
      type: 'uniform',
      lower: 0,
      upper: 0
  ...
  if ($(item.metadata.render.response.direct.headers))
    item.metadata.render.response.direct.headers = []
  ...
  Object.keys(tmpResponseHeaders).forEach(key => {
    ...
    if (headerValue && Object.prototype.toString.call(headerValue) === '[object Array]') {
      //多值 Header
  ...
  // Response Body
  item.metadata.render.response.direct.bodyType =
    item.metadata.wmui.responseBodyType ||
  ...
  if ($(item.response.additionalProxyRequestHeaders))
    item.response.additionalProxyRequestHeaders = {}
  if ($(item.metadata.render.response.proxy)) item.metadata.render.response.proxy = {}
  if ($(item.metadata.render.response.proxy.headers))
    item.metadata.render.response.proxy.headers = []
  ...
  ;(item.postServeActions || []).forEach((webhook, index) => {
    ...
    item.metadata.render.postServeActions[index].parameters.delayType =
      (webhook.parameters.delay && webhook.parameters.delay.type) || 'none'
    if ($(webhook.parameters.delay))
      webhook.parameters.delay = {
        type: 'fixed'
      }
...
// 转换单个匹配数据
function convertSinglePredicateData(
  singlePredicateData: any,
  ...
  for (let key in singlePredicateData) {
    ...
    if ($enum.ALL_PREDICATE_COLLECTION.includes(key)) {
      // predicate
      item.predicate = key
      if (['[object String]', '[object Boolean]'].includes(Object.prototype.toString.call(value))) {
        item.value = value
        if (key === 'absent') {
          //el-input 不支持绑定 boolean 类型变量，此处做下类型转换
