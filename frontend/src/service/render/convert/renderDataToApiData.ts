...
export const renderDataToApiData = (item: IStubMapping) => {
  ...
  if (
    item.metadata.render.request.queryParameters &&
    item.metadata.render.request.queryParameters.length
  ) {
    res.request.queryParameters = renderDataToApiDataWithMatch(
      item.metadata.render.request.queryParameters,
      {}
    )
  }
  ...
  switch (item.metadata.render.request.bodyType) {
    case 'raw':
      res.request.bodyPatterns = renderDataToApiDataWithMatch(
        item.metadata.render.request.bodyPatterns,
        []
      )
      break
    case 'form-data':
    ...
    case 'x-www-form-urlencoded':
      res.request.formParameters = renderDataToApiDataWithMatch(
        item.metadata.render.request.formParameters,
        {}
      )
      break
    default:
  ...
  switch (item.metadata.render.response.responseType) {
    ...
    case 'direct':
      ...
      if (
        ...
        item.metadata.render.response.direct.headers.length
      ) {
        const headers: {
  ...
  switch (item.metadata.render.response.delayType) {
    ...
    case 'chunkedDribbleDelay':
      res.response.chunkedDribbleDelay = JSON.parse(
        JSON.stringify(item.response.chunkedDribbleDelay)
      )
      break
    case 'lognormal':
  ...
  if (
    item.response.transformerParameters &&
    Object.keys(item.response.transformerParameters).length
  ) {
...
export function renderDataToApiDataWithMatch(
  renderDataWithMatch: MatchNodeData[],
  result: any
): any {
