<template>
  <template v-for="(item, index) in modelValue" :key="item.id">
    ...
    <el-form-item
      :class="
        item.params !== undefined && item.params !== null && item.params.length
          ? 'has-match-params'
          : ''
      "
    >
      ...
      <el-input
        v-model="item.key"
        class="match-item-value match-input"
        v-if="item.key !== undefined && item.key !== null"
      ></el-input>
      <el-select
        v-model="item.predicate"
        class="match-item-value match-select"
        @change="(predicate: string) => onChangePredicate(item, predicate)"
      >
        <el-option
          v-for="predicate in $enum.ALL_PREDICATE_COLLECTION"
          :key="predicate"
          :value="predicate"
          :label="predicate"
        ></el-option>
        ...
        <template v-if="Object.prototype.toString.call(item.value) === '[object Boolean]'">
          <el-input disabled v-model="item.extra.strValue" class="match-item-value match-input">
            <template #suffix>
              <el-switch
                v-model="item.value"
                @change="(val: boolean) => (item.extra.strValue = val.toString())"
              ></el-switch>
            </template>
          </el-input>
        </template>
      ...
      <el-button
        :icon="Plus"
        type="success"
        v-if="$enum.MULTI_SUBPREDICATE_COLLECTION.includes(item.predicate)"
        @click="addPredicate(item)"
      ></el-button>
      <el-button
        :icon="Delete"
        type="danger"
        v-if="
          item.parentPredicate == null ||
          $enum.MULTI_SUBPREDICATE_COLLECTION.includes(item.parentPredicate)
        "
        @click="deletePredicate(index)"
      ></el-button>
    </el-form-item>
    <!-- Predicate Parameters -->
    <el-form-item
      size="small"
      v-if="item.params !== undefined && item.params !== null && item.params.length"
      :class="
        item.parentPredicate === undefined || item.parentPredicate === null
          ? 'match-params match-params-root'
          : 'match-params match-params-sub'
      "
    >
      <el-form-item v-for="param in item.params" class="match-params-item">
        ...
        <template v-else-if="param.key === 'subPredicate'">
          <el-checkbox
            ...
            @change="(newVal: boolean) => switchPredicateMode(item, newVal)"
          ></el-checkbox>
        </template>
        <template v-else>
          <el-checkbox label="" v-model="param.enabled" style="margin-right: 8px"></el-checkbox>
          <el-input
            v-if="$enum.INPUT_PARAM_COLLECTION.includes(param.key)"
            v-model="param.value"
            class="match-input"
            :style="getParamsComponentWidth(param.key)"
          >
            <template #prefix
              ><span style="padding-right: 5px">{{ param.key }}</span></template
            >
          </el-input>
          <el-select
            v-if="$enum.SINGLE_SELECT_PARAM_COLLECTION.includes(param.key)"
            v-model="param.value"
            placeholder="Please Select"
            :style="getParamsComponentWidth(param.key)"
          >
            <el-option
              v-for="option in $enum.PREDICATE_PARAM_OPTIONS_COLLECTION[param.key]"
              :key="option"
              :label="option"
              :value="option"
            />
          ...
          <el-select
            v-model="param.value"
            v-if="$enum.MULTI_SELECT_PARAM_COLLECTION.includes(param.key)"
            multiple
            collapse-tags
          ...
          <el-button
            v-if="param.key === 'xPathNamespaces'"
            @click="isShowxPathNamespacesDialog[item.id] = true"
            class="xPathNamespaces"
            >{{ param.key }}</el-button
          >
          <template v-if="isShowxPathNamespacesDialog[item.id]">
            <el-dialog
              v-model="isShowxPathNamespacesDialog[item.id]"
              title="xPathNamespaces"
              center
              top="30vh"
            ...
            >
              <div class="addNamespaceButton">
                <el-button
                  ...
                  :icon="Plus"
                  size="small"
                  @click="addNamespaceItem(param.value as [])"
                ></el-button>
              </div>
                ...
                <el-form-item style="flex: 1; padding-bottom: 10px">
                  ...
                  <el-col :span="2" style="text-align: right">
                    <el-button
                      ...
                      :icon="Delete"
                      size="small"
                      @click="deleteNamespaceItem(param.value as [], index)"
                    ></el-button>
                  </el-col>
                </el-form-item>
              </template>
